...
const jwksUrl = 'https://dev-nkoq-jta.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
    ...
    const signedKeys = resp.data.keys
    const signingKey = signedKeys.find(key => key.kid === jwt.header.kid);
    const cert = await certToPEM(signingKey.x5c[0])
